...
class TestCatalogModule(IntegrationTestCase):
    ...
    def test_addOrUpdateIndexes(self):
        ...
        priorDocument = api.content.create(type="Document", id="prior-document", container=self.portal)
    ...
    def test_addOrUpdateSeveralIndexes(self):
        ...
        addOrUpdateIndexes(
            ...
            OrderedDict([("reversedUID", ("FieldIndex", {})), ("UID", ("UUIDIndex", {}))]),
        )
        self.assertTrue("reversedUID" in self.portal.portal_catalog.indexes())
        # moreover existing objects were updated
        brains = self.catalog(reversedUID=priorDocument.UID()[::-1])
    ...
    def test_addOrUpdateColumnsWithExistingIndex(self):
        ...
        # add a document prior to adding the index
        api.content.create(type="Document", id="prior-document", container=self.portal)
        addOrUpdateIndexes(self.portal, {"reversedUID": ("FieldIndex", {})})
    ...
    def test_merge_queries(self):
        ...
        self.assertEqual(
            merge_queries([query1, query2]),
            {"portal_type": {"query": ["Document"]}, "review_state": {"query": ["private", "published"]}},
        )
        self.assertEqual(
            merge_queries([query1, query3]),
            {
        ...
        self.assertEqual(
            merge_queries([query2, query4]),
            {"portal_type": {"query": ["Document"]}, "review_state": {"query": ["published", "pending"]}},
        )
        self.assertEqual(
            merge_queries([query1, query2, query3, query4, query5]),
            {
                "portal_type": {"query": ["Document", "Folder"]},
