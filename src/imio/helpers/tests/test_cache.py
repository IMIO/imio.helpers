...
class TestCacheModule(IntegrationTestCase):
    ...
    def test_cleanVocabularyCacheFor(self):
        ...
        vocab = queryUtility(IVocabularyFactory, "imio.helpers.testing.testingvocabulary")
        # not cached for now
        self.assertFalse(getattr(vocab, memPropName, {}))
        # once get, it is cached
        vocab(self.portal)
        self.assertTrue(getattr(vocab, memPropName))
        self.assertEqual([term.token for term in vocab(self.portal)._terms], ["1", "2"])
        ...
        self.assertEqual([term.token for term in vocab(self.portal)._terms], ["1", "2", "3"])
        ...
        # clean vocabulary cache
        cleanVocabularyCacheFor("imio.helpers.testing.testingvocabulary")
        self.assertEqual([term.token for term in vocab(self.portal)._terms], ["1", "2", "3", "4"])
        ...
        self.assertEqual([term.token for term in vocab(self.portal)._terms], ["1", "2", "3", "4", "5"])
...
class TestCachedMethods(IntegrationTestCase):
    ...
    def setUp(self):
        ...
        api.user.create(
            "a@b.be", "user1", "123-45@6U78", properties={"fullname": "Stéphan Smith"}
        )  # Returns MemberData
    ...
    def test_getGroupsForPrincipal(self):
        ...
        self.assertListEqual(pgr.getGroupsForPrincipal(self.user), ["AuthenticatedUsers"])
        pgr.addPrincipalToGroup("user1", "Administrators")
        self.assertListEqual(pgr.getGroupsForPrincipal(self.user), ["Administrators", "AuthenticatedUsers"])
        api.group.add_user(groupname="Reviewers", user=self.user)
        self.assertListEqual(
            pgr.getGroupsForPrincipal(self.user), ["Administrators", "Reviewers", "AuthenticatedUsers"]
        )
        api.group.remove_user(groupname="Administrators", user=self.user)
        self.assertListEqual(pgr.getGroupsForPrincipal(self.user), ["Reviewers", "AuthenticatedUsers"])
    ...
    def test_getRolesForPrincipal(self):
        ...
        self.assertTupleEqual(prm.getRolesForPrincipal(self.user), ("Member",))
        prm.assignRolesToPrincipal(
            (
                "Member",
                "Reviewer",
            ),
        ...
        self.assertTupleEqual(sorted_roles, ("Member", "Reviewer"))
        prm.assignRoleToPrincipal("Contributor", "user1")
        sorted_roles = tuple(sorted(prm.getRolesForPrincipal(self.user), key=str.lower))
        self.assertTupleEqual(sorted_roles, ("Contributor", "Member", "Reviewer"))
        prm.removeRoleFromPrincipal("Reviewer", "user1")
        sorted_roles = tuple(sorted(prm.getRolesForPrincipal(self.user), key=str.lower))
        self.assertTupleEqual(sorted_roles, ("Contributor", "Member"))

    def test__getGroupsForPrincipal(self):
        self.skipTest("Temporary skipped")
        pgr = self.portal["portal_groups"]
        self.assertListEqual(self.acl._getGroupsForPrincipal(self.user), ["AuthenticatedUsers"])
        pgr.addPrincipalToGroup("user1", "Administrators")
        self.assertListEqual(self.acl._getGroupsForPrincipal(self.user), ["Administrators", "AuthenticatedUsers"])
        api.group.add_user(groupname="Reviewers", user=self.user)
        invalidate_cachekey_volatile_for("_users_groups_value", get_again=True)
        self.assertListEqual(
            self.acl._getGroupsForPrincipal(self.user), ["Administrators", "Reviewers", "AuthenticatedUsers"]
        )
        api.group.remove_user(groupname="Administrators", user=self.user)
        self.assertListEqual(self.acl._getGroupsForPrincipal(self.user), ["Reviewers", "AuthenticatedUsers"])
        pgr.removePrincipalFromGroup("user1", "Reviewers")
        self.assertListEqual(self.acl._getGroupsForPrincipal(self.user), ["AuthenticatedUsers"])
    ...
    def test__listAllowedRolesAndUsers(self):
        ...
        self.assertEqual(
            ...
            ["user:new_user", "Member", "Authenticated", "user:AuthenticatedUsers", "Anonymous"],
        )
        ...
        self.assertEqual(self.catalog._listAllowedRolesAndUsers(anon_user), ["Anonymous"])
