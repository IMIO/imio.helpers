...
class TestContentModule(IntegrationTestCase):
    ...
    def test_get_schema_fields(self):
        ...
        obj = api.content.create(
            container=self.portal.folder, id="tt", type="testingtype", enabled="Should be a boolean"
        )
        ...
        self.assertListEqual(
            ...
            sorted(
                [
                    ...
                    "ITALCondition.tal_condition",
                    "ITALCondition.roles_bypassing_talcondition",
                ]
            ),
        )
    ...
    def test_validate_fields(self):
        ...
        self.assertEqual(validate_fields(obj), [WrongType("Should be a boolean", bool, "enabled")])
        obj.enabled = False
        ...
        if six.PY3:
            self.assertEqual(validate_fields(obj), [WrongType(None, str, "mandatory_textline")])
        else:
            self.assertEqual(validate_fields(obj), [WrongType(None, six.text_type, "mandatory_textline")])
    ...
    def test_uuidsToCatalogBrains(self):
        ...
        self.assertEqual(len(uuidsToCatalogBrains(uuids)), 2)
        self.assertEqual([brain.UID for brain in uuidsToCatalogBrains(uuids, ordered=True)], [folder_uid, folder2_uid])
        uuids = [folder2_uid, folder_uid]
        self.assertEqual([brain.UID for brain in uuidsToCatalogBrains(uuids, ordered=True)], uuids)
    ...
    def test_uuidsToObjects(self):
        ...
        self.assertEqual(uuidsToObjects(uuids, ordered=True), [self.portal.folder, self.portal.folder2])
        uuids = [folder2_uid, folder_uid]
        self.assertEqual(uuidsToObjects(uuids, ordered=True), [self.portal.folder2, self.portal.folder])
    ...
    def test_get_relations(self):
        ...
        self.assertTrue(isinstance(list(get_relations(obj, "relations", as_obj=True))[0], self.portal.folder.__class__))
    ...
    def test_normalize_name(self):
        ...
        self.assertEqual(normalize_name(request, u"CV Informaticien N\xb02016-1"), "cv-informaticien-ndeg2016-1")
        self.assertEqual(normalize_name(request, u"Héhé du texte"), "hehe-du-texte")
    ...
    def test_at_object_values(self):
        ...
        self.assertEqual(object_values(self.portal, ["Folder", "ATFolder"]), [self.portal.folder, self.portal.folder2])
    ...
    def test_at_object_ids(self):
        ...
        # may pass a list of class names
        self.assertEqual(object_ids(self.portal, ["Folder", "ATFolder"]), ["folder", "folder2"])
    ...
    def test_sort_on_vocab_order(self):
        self.assertEqual(
            sort_on_vocab_order(
                ("testingtype", "Link", "Document", "File"), vocab_name="plone.app.vocabularies.PortalTypes"
            ),
            ["File", "Link", "Document", "testingtype"],
        )
        vocab = get_vocab(self.portal, "plone.app.vocabularies.PortalTypes")
        self.assertEqual(
            sort_on_vocab_order(("testingtype", "Link", "Document", "File"), vocab=vocab),
            ["File", "Link", "Document", "testingtype"],
        )
        # not found values are moved to the end
        self.assertEqual(
            sort_on_vocab_order(("testingtype", "unknown1", "Link", "Document", "File"), vocab=vocab),
            ["File", "Link", "Document", "testingtype", "unknown1"],
        )
        self.assertEqual(
            sort_on_vocab_order(("testingtype", "unknown2", "Link", "Document", "File", "unknown1"), vocab=vocab),
