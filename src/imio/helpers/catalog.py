    ...
    for indexName, indexInfo in indexInfos.items():
        ...
        if indexName in zopeCatalog.indexes:
            ...
            if oldType != indexType or (
                ...
                and (storedIndex.lexicon_id != extra.lexicon_id or storedIndex._index_type != extra.index_type)
            ):
                needToDeleteIndex = True
            if needToDeleteIndex:
                catalog.delIndex(indexName)
                logger.info(
                    'Existing index "%s" of type "%s" was removed:'
                    ' we need to recreate it with type "%s".' % (indexName, oldType, indexType)
                )
    ...
    for query in queries:
        for k, v in query.items():
            ...
            if k not in res:
                ...
                if subkey in res[k]:
                    # avoid duplicates
                    value = [_v for _v in value if _v not in res[k][subkey]]
